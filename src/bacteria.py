...
class bacterium(object):    
    ...
    def split(self):
        ...
        spawn_angle = [self.angle[0]+(0.5-random.random())*3.1415*0.5, self.angle[1]+(0.5-random.random())*3.1415*0.5]
        self.angle  = [self.angle[0]+(0.5-random.random())*3.1415*0.5, self.angle[1]+(0.5-random.random())*3.1415*0.1]
        ...
        pos = (self.pos[0]+spawn_offsetX*volumeRatio, self.pos[1]+spawn_offsetY*volumeRatio, self.pos[2]+spawn_offsetZ*volumeRatio)
    ...
    def getPositions(self):
        ...
        positions.append((self.pos[0]-int(0.1*dx_length), self.pos[1]-int(0.1*dy_length), self.pos[2]-int(0.1*dz_length)))
        for index in range(-int(self.length*0.01), int(self.length*0.01)+1):
            positions.append((self.pos[0]+int(10*index*math.sin(self.angle[0])*math.cos(self.angle[1])), self.pos[1]+int(10*index*math.cos(self.angle[0])*math.cos(self.angle[1])), self.pos[2]+int(10*index*math.sin(self.angle[1]))))
        positions.append((self.pos[0]+int(0.1*dx_length), self.pos[1]+int(0.1*dy_length), self.pos[2]+int(0.1*dz_length)))
        return positions
        ...
        for position in self.getPositions():
            if(False):
                ...
                if(position[0]<-xborder+self.width):
                    ...
                    self.totalForce_equivalent = self.totalForce_equivalent + absolute(-position[0]-xborder+self.width)*0.1
                if(position[0]>xborder-self.width):
                    self.velocity[0] = self.velocity[0] + (-position[0]+xborder-self.width)*0.1   
                    self.totalForce_equivalent = self.totalForce_equivalent + absolute(-position[0]+xborder-self.width)*0.1                    
            ...
            if(position[2]<-0+self.width):
                ...
                for index in range(lenPos-1):
                    ...
                    self.velocity_angular[1] = self.velocity_angular[1] - t_radius * math.cos(self.angle[1])*repulsion_z/lenPos*0.05*interactionfactor*dt    
                ...
                if(dr**0.5<(self.width)*1.05*5*far_away_factor):
                    ...
                    for _index in range(_lenPos-1):
                        ...
                        if (dx != 0) or (dy != 0) or  (dz != 0):
                            ...
                            #
                            self.velocity[0]  =  self.velocity[0] + int(repulsion_x/lenPos**(1/2)*interactionfactor)
                            self.velocity[1]  =  self.velocity[1] + int(repulsion_y/lenPos**(1/2)*interactionfactor)
                            self.velocity[2]  =  self.velocity[2] + int(repulsion_z/lenPos**(1/2)*interactionfactor)
                            #add torque
                            t_radius = (index-lenPos*0.5)
                            self.velocity_angular[0] = self.velocity_angular[0] + t_radius * math.cos(self.angle[0])* math.cos(self.angle[1])*repulsion_x/lenPos*0.05*interactionfactor
                            self.velocity_angular[0] = self.velocity_angular[0] - t_radius * math.sin(self.angle[0])* math.cos(self.angle[1])*repulsion_y/lenPos*0.05*interactionfactor     
                            #Torque on second angle  
                            self.velocity_angular[1] = self.velocity_angular[1] + t_radius * math.cos(self.angle[1])*(repulsion_x**2+repulsion_y**2)**(1/2)/lenPos*0.0125*interactionfactor
                            self.velocity_angular[1] = self.velocity_angular[1] + t_radius * math.sin(self.angle[1])*repulsion_z/lenPos*0.05*interactionfactor         
                            self.totalForce_equivalent  = self.totalForce_equivalent  + absolute(repulsion_x/lenPos*interactionfactor)
                            self.totalForce_equivalent  = self.totalForce_equivalent  + absolute(repulsion_y/lenPos*interactionfactor)
                            
                            #Actio-Reactio                  
                            _Bacterium.velocity[0] = _Bacterium.velocity[0] - (repulsion_x/lenPos**(1/2)*interactionfactor)
                            _Bacterium.velocity[1] = _Bacterium.velocity[1] - (repulsion_y/lenPos**(1/2)*interactionfactor)
                            _Bacterium.velocity[2] = _Bacterium.velocity[2] - (repulsion_z/lenPos**(1/2)*interactionfactor)
                            _Bacterium.totalForce_equivalent  = _Bacterium.totalForce_equivalent  + absolute(repulsion_y/lenPos*interactionfactor)
                            _Bacterium.totalForce_equivalent  = _Bacterium.totalForce_equivalent  + absolute(repulsion_x/lenPos*interactionfactor)
